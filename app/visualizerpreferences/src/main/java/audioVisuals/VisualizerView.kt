...
open class VisualizerView(
   ...
   // @ColorInt
    ...
    private fun calcCurrentAngle(): Double {
        ...
        val revolutions =
            elapsedTime * REVOLUTIONS_PER_SECOND / 1000
    ...
    fun updateFFT(bytes: ByteArray) {
        ...
        run {
            ...
            while (i < bytes.size * BASS_SEGMENT_SIZE) {
                //bassTotal += Math.abs(bytes[i]).toFloat()
                i++
            }
        }
        ...
        run {
            ...
            while (i < bytes.size * MID_SEGMENT_SIZE) {
               // midTotal += Math.abs(bytes[i]).toFloat()
                i++
            }
        }
        ...
        for (i in (bytes.size * MID_SEGMENT_SIZE).toInt() until bytes.size) {
            //trebleTotal += Math.abs(bytes[i]).toFloat()
        }
    ...
    fun setColor(newColorKey: String) {
        ...
        when (newColorKey) {
            context.getString(R.string.pref_color_blue_value) -> {
                ...
                //      backgroundColor = ContextCompat.getColor(context, R.color.backgroundBlue)
            }
            context.getString(R.string.pref_color_green_value) -> {
                ...
                //    backgroundColor = ContextCompat.getColor(context, R.color.backgroundGreen)
            }
            else -> {
                shapeColor = ContextCompat.getColor(context, R.color.shapeRed)
                trailColor = ContextCompat.getColor(context, R.color.trailRed)
                //  backgroundColor = ContextCompat.getColor(context, R.color.backgroundRed)
            }
        }
    ...
    init {
        ...
        mBassCircle = object : TrailedShape(BASS_MULTIPLIER) {
            ...
            override fun drawThisShape(
                ...
                canvas?.drawCircle(shapeCenterX.toFloat(), shapeCenterY.toFloat(), currentSize, paint!!)
        ...
        mMidSquare = object : TrailedShape(MID_MULTIPLIER) {
            ...
            override fun drawThisShape(
                ...
                canvas?.drawRect(
                    (shapeCenterX - currentSize).toFloat(),
                    (shapeCenterY - currentSize).toFloat(),
                    (shapeCenterX + currentSize).toFloat(),
                    (shapeCenterY + currentSize).toFloat(),
                    paint!!
                )
        ...
        mTrebleTriangle = object : TrailedShape(TREBLE_MULTIPLIER) {
            ...
            override fun drawThisShape(
                ...
                trianglePath.moveTo(shapeCenterX.toFloat(), (shapeCenterY - currentSize).toFloat())
                trianglePath.lineTo((shapeCenterX + currentSize).toFloat(),
                    (shapeCenterY + currentSize / 2).toFloat()
                )
                trianglePath.lineTo((shapeCenterX - currentSize).toFloat(),
                    (shapeCenterY + currentSize / 2).toFloat()
                )
                trianglePath.lineTo(shapeCenterX.toFloat(), (shapeCenterY - currentSize).toFloat())
