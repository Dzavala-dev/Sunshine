...
class WeatherProvider : ContentProvider() {
    ...
    override fun bulkInsert(
        ...
        return when (WeatherProvider.Companion.sUriMatcher.match(uri)) {
            WeatherProvider.Companion.CODE_WEATHER -> {
                db.beginTransaction()
                var rowsInserted = 0
                try {
                    for (value in values) {
                        ...
                        require(SunshineDateUtils.isDateNormalized(weatherDate)) { "Date must be normalized to insert" }
                        val _id =
    ...
    override fun query(
        ...
        val cursor: Cursor
        cursor = when (WeatherProvider.Companion.sUriMatcher.match(uri)) {
            WeatherProvider.Companion.CODE_WEATHER_WITH_DATE -> {
                ...
                val selectionArguments =
                    arrayOf(normalizedUtcDateString)
                mOpenHelper!!.readableDatabase.query( /* Table we are going to query */
    ...
    override fun delete(
        ...
        numRowsDeleted = when (WeatherProvider.Companion.sUriMatcher.match(uri)) {
            WeatherProvider.Companion.CODE_WEATHER -> mOpenHelper!!.writableDatabase.delete(
                WeatherContract.WeatherEntry.TABLE_NAME,
                selection,
                selectionArgs
            )
        ...
        /* If we actually deleted any rows, notify that a change has occurred to this URI */if (numRowsDeleted != 0) {
            context!!.contentResolver.notifyChange(uri, null)
        }
     ...
     */
        private val sUriMatcher: UriMatcher = WeatherProvider.Companion.buildUriMatcher()
